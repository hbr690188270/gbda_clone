...
def main(args):
    ...
    if pretrained and args.dataset == "mnli":
        # label_perm = lambda x: (x + 1) % 3
        label_perm = transform_mnli
    ...
    for idx in tqdm.tqdm(range(len(selected_idx_list))):
        curr_idx = selected_idx_list[idx]
        input_ids = encoded_dataset[testset_key]['input_ids'][curr_idx]
        # input_ids = encoded_dataset[testset_key]['input_ids'][idx]
        if args.model == 'gpt2':
            token_type_ids = None
        else:
            # token_type_ids = encoded_dataset[testset_key]['token_type_ids'][idx]
            token_type_ids = encoded_dataset[testset_key]['token_type_ids'][curr_idx]

        # label = label_perm(encoded_dataset[testset_key]['label'][idx])
        label = label_perm(encoded_dataset[testset_key]['label'][curr_idx])
        ...
        try:
            ...
            if torch.argmax(clean_logit.view(-1)) != label:
                print("wrong predition,  skip this example!\n")
                continue
            ...
            if args.dataset == 'mnli':
                ...
                premise_length = len(tokenizer.encode(encoded_dataset[testset_key]['premise'][curr_idx]))
            ...
            if args.dataset == 'mnli':
                ...
                print('%s \n%s' % (clean_premise, clean_hypothesis))
            else:
            ...
            with torch.no_grad():
                for j in range(args.gumbel_samples):
                    ...
                    adv_label = adv_logit.argmax().item()
            ...
            if type(adv_ids) != list:
                adv_token_ids.append(adv_ids.tolist())
            else:
                adv_token_ids.append(adv_ids)
            orig_token_ids.append(input_ids)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="dbpedia14", type=str,
        choices=["dbpedia14", "ag_news", "imdb", "yelp", "mnli", 'sst'],
